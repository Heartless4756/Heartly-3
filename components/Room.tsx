...
import { 
  ...
  Lock, Unlock, LogOut, ArrowUpCircle, UserPlus, X as XIcon, ChevronDown,
  ChevronLeft, MoreHorizontal, Volume2, Gift, LayoutGrid, Plus, Eye,
  Share2, Flag, Minimize2, Loader2,
  PenTool, Eraser, Download, Trash2, Check, RotateCcw, Power, Users,
  Play, Upload, Headphones, Disc3, Music2, Pause, ListMusic, SkipForward,
  ShieldAlert, ShieldCheck, VolumeX, UserCheck, Ban, Maximize2, ArrowRight, ArrowUp,
  Search, UserX, Settings, Smile, Image as ImageIcon,
  CheckCircle2, Square, Circle, Minus as LineIcon, Grid, Type as TextIcon, Move, 
  Palette, MousePointer2, SlidersHorizontal, Undo2, StickyNote
} from 'lucide-react';
...
interface Stroke {
  ...
  opacity: number;
  isEraser: boolean;
  type: 'pen' | 'rect' | 'circle' | 'line' | 'stamp';
  stampIcon?: string;
  order: number;
}
...
const COLORS = ['#000000', '#FF0000', '#00FF00', '#0000FF', '#FFFF00', '#FF00FF', '#00FFFF', '#FFFFFF', '#FF8C00', '#8B4513'];
const STAMPS = ['⭐', '🔥', '❤️', '👑', '💎', '🎨', '✨', '👍', '🐱', '🌹'];
...
const SettingsSection: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
  <div className="mb-6"><label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{title}</label>{children}</div>
);
...
export const ActiveRoom: React.FC<RoomProps> = ({ roomId, currentUser, onLeave, isMinimized, onMinimize }) => {
    ...
    const [showDrawUserSelect, setShowDrawUserSelect] = useState(false);
    const [showDrawingMenu, setShowDrawingMenu] = useState(false);
    const [isBoardHidden, setIsBoardHidden] = useState(false);
    const [isBoardFullscreen, setIsBoardFullscreen] = useState(false);
    const [boardPattern, setBoardPattern] = useState<'none' | 'grid' | 'dots'>('none');
    
    // Drawing UI State
    const [brushColor, setBrushColor] = useState('#000000');
    const [brushSize, setBrushSize] = useState(5);
    const [brushOpacity, setBrushOpacity] = useState(1);
    const [drawMode, setDrawMode] = useState<'pen' | 'rect' | 'circle' | 'line' | 'stamp'>('pen');
    ...
    useEffect(() => {
        ...
        const init = async () => {
          ...
          const strokesQuery = query(collection(db, 'rooms', roomId, 'strokes'), orderBy('order', 'asc'));
          const unsubStrokes = onSnapshot(strokesQuery, (snapshot) => { setStrokes(snapshot.docs.map(d => ({ id: d.id, ...d.data() } as Stroke))); });
          unsubscribersRef.current.push(unsubStrokes);
    ...
    const drawGrid = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
        if (boardPattern === 'none') return;
        ctx.save();
        ctx.strokeStyle = '#E2E8F0';
        ctx.lineWidth = 1;
        if (boardPattern === 'grid') {
            const size = 30;
    ...
    const renderStroke = (ctx: CanvasRenderingContext2D, stroke: Stroke) => {
        ...
        ctx.lineCap = 'round';
        ctx.lineJoin = 'round';
        ctx.strokeStyle = stroke.isEraser ? '#FFFFFF' : (stroke.color || '#000000');
        ctx.globalAlpha = stroke.isEraser ? 1 : (stroke.opacity ?? 1);
        ctx.lineWidth = stroke.size || 5;

        const p0 = stroke.points[0];
        const pN = stroke.points[stroke.points.length - 1];
        if (!p0 || !pN) { ctx.restore(); return; }
        ...
        if (stroke.type === 'pen') {
            ...
            const radius = Math.sqrt(Math.pow(pN.x - p0.x, 2) + Math.pow(pN.y - p0.y, 2));
            ctx.beginPath();
            ...
            ctx.font = `${stroke.size * 2}px serif`;
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ctx.fillText(stroke.stampIcon || '🔥', p0.x, p0.y);
        }
        ctx.restore();
    };

    useEffect(() => {
        if (!roomData?.drawingState?.isActive || isBoardHidden || isMinimized) return;
    ...
    }, [strokes, roomData?.drawingState?.isActive, isBoardHidden, isMinimized, boardPattern]);
    ...
    const startDrawing = (e: React.MouseEvent | React.TouchEvent) => { 
        ...
        // If stamp mode, just place it immediately on start
        if (drawMode === 'stamp') {
            placeStamp(startPoint);
            isDrawingRef.current = false;
        }
    };

    const placeStamp = async (point: Point) => {
        await addDoc(collection(db, 'rooms', roomId, 'strokes'), { 
            points: [point], 
            color: brushColor || '#000000', 
            size: brushSize || 20, 
            opacity: brushOpacity ?? 1,
            isEraser: false, 
            type: 'stamp',
    ...
    const draw = (e: React.MouseEvent | React.TouchEvent) => {
          if (!isDrawingRef.current || !isArtist || drawMode === 'stamp') return;
          e.preventDefault(); 
          ...
          if (ctx && currentStrokeRef.current.length > 0) {
              if (drawMode === 'pen') {
                  ...
                  if (lastPoint) {
                    ctx.save();
                    ctx.beginPath();
                    ctx.strokeStyle = isEraser ? '#FFFFFF' : (brushColor || '#000000');
                    ctx.globalAlpha = isEraser ? 1 : (brushOpacity ?? 1);
                  ...
                  const startPoint = currentStrokeRef.current[0];
                  if (startPoint) {
                    currentStrokeRef.current = [startPoint, point];
                    // Immediate visual feedback without flickering
                    ctx.clearRect(0, 0, canvas!.width, canvas!.height);
    ...
    const stopDrawing = async () => {
          ...
          if (currentStrokeRef.current.length > 0) { 
              await addDoc(collection(db, 'rooms', roomId, 'strokes'), { 
                  ...
                  opacity: brushOpacity ?? 1,
                  isEraser: isEraser, 
                  type: (isEraser ? 'pen' : drawMode) || 'pen',
                  order: Date.now() 
              }); 
          }
    ...
    const clearCanvas = async () => {
          if(!window.confirm("Clear board?")) return;
          const batch = writeBatch(db);
          const snapshot = await getDocs(collection(db, 'rooms', roomId, 'strokes'));
          snapshot.docs.forEach(doc => batch.delete(doc.ref));
          await batch.commit(); 
          setShowDrawingMenu(false);
    };
    
    const disableDrawing = async () => {
          if (!roomData) return;
          await updateDoc(doc(db, 'rooms', roomId), { 'drawingState.isActive': false, 'drawingState.artistId': null, 'drawingState.invitePending': null });
          setShowDrawingMenu(false);
          setIsBoardFullscreen(false);
    };

    const undoLastStroke = async () => {
        if (!isArtist || strokes.length === 0) return;
        const lastStroke = strokes[strokes.length - 1];
        if (lastStroke && lastStroke.id) await deleteDoc(doc(db, 'rooms', roomId, 'strokes', lastStroke.id));
    };
    
    const handleChangePainterClick = () => { setShowDrawingMenu(false); setShowDrawUserSelect(true); };
    ...
    const selectDrawingUser = async (userToSelect: Participant) => { if (roomData) { if (userToSelect.uid === currentUser.uid) await updateDoc(doc(db, 'rooms', roomId), { 'drawingState': { isActive: true, artistId: currentUser.uid, artistName: currentUser.displayName, invitePending: null } }); else await updateDoc(doc(db, 'rooms', roomId), { 'drawingState.invitePending': { to: userToSelect.uid, toName: userToSelect.displayName } }); setShowDrawUserSelect(false); } };
    const acceptDrawingInvite = async () => { await updateDoc(doc(db, 'rooms', roomId), { 'drawingState': { isActive: true, artistId: currentUser.uid, artistName: currentUser.displayName, invitePending: null } }); };
    const declineDrawingInvite = async () => { await updateDoc(doc(db, 'rooms', roomId), { 'drawingState.invitePending': null }); };
    const downloadCanvas = () => { const canvas = canvasRef.current; if (canvas) { const link = document.createElement('a'); link.download = 'drawing.png'; link.href = canvas.toDataURL(); link.click(); } };
    ...
    const isDrawingActive = roomData?.drawingState?.isActive;
    const isArtist = roomData?.drawingState?.artistId === currentUser.uid;
    const drawingInvite = roomData?.drawingState?.invitePending;
    const isDrawingPendingMe = drawingInvite?.to === currentUser.uid;
    ...
    return (
        <div className={`h-full w-full bg-[#050505] flex flex-col relative transition-all duration-300 ${isMinimized ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
            ...
            <div className="flex items-center justify-between px-4 py-4 relative z-20">
                <div className="flex items-center gap-2">
                    ...
                    <div className="text-left flex flex-col justify-center">
                        ...
                        <p className="text-gray-200 text-[10px] font-bold drop-shadow-md">{participants.length} Online</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
            ...
            <div className="relative flex-1 overflow-hidden z-10 flex flex-col">
                 ...
                 {isDrawingActive && !isBoardHidden && (
                    <div className={`absolute z-20 transition-all duration-500 flex flex-col p-4 ${isBoardFullscreen ? 'inset-0' : 'top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-2xl aspect-video'}`}>
                        <div className="flex-1 bg-white rounded-3xl overflow-hidden shadow-[0_30px_60px_rgba(0,0,0,0.5)] border-4 border-[#1A1A23] flex flex-col relative">
                            {/* Canvas Header */}
                            <div className="bg-[#1A1A23] px-4 py-2 flex items-center justify-between text-white border-b border-white/10">
                                <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-full bg-violet-500/20 flex items-center justify-center text-violet-400">
                                        <Palette size={16} />
                                    </div>
                                    <span className="text-[10px] font-bold uppercase tracking-widest">Artist: {roomData?.drawingState?.artistName}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <button onClick={() => setBoardPattern(prev => prev === 'none' ? 'grid' : prev === 'grid' ? 'dots' : 'none')} className="p-2 hover:bg-white/10 rounded-xl" title="Toggle Pattern"><Grid size={16} /></button>
                                    <button onClick={() => setIsBoardFullscreen(!isBoardFullscreen)} className="p-2 hover:bg-white/10 rounded-xl">{isBoardFullscreen ? <Minimize2 size={16} /> : <Maximize2 size={16} />}</button>
                                    {(isHost || isAdmin) && <button onClick={downloadCanvas} className="p-2 hover:bg-white/10 rounded-xl"><Download size={16}/></button>}
                                    <button onClick={() => setIsBoardHidden(true)} className="p-2 hover:bg-white/10 rounded-xl text-gray-400"><XIcon size={16}/></button>
                                    {(isHost || isAdmin) && <button onClick={clearCanvas} className="p-2 bg-red-500/10 hover:bg-red-500/20 rounded-xl text-red-400"><Trash2 size={16}/></button>}
                                </div>
                            </div>
                            ...
                            <div className="flex-1 relative bg-white">
                                <canvas 
                                    ...
                                    className={`w-full h-full touch-none ${isArtist ? 'cursor-crosshair' : 'cursor-default'}`} 
                                />
                            </div>

                            {/* Artist Floating Controls */}
                            {isArtist && (
                                <div className="absolute left-1/2 -translate-x-1/2 bottom-4 flex flex-col items-center gap-3 z-30">
                                    {/* Tool Settings Tooltip */}
                                    {(showBrushSettings || showStampPicker) && (
                                        <div className="bg-[#1A1A23]/95 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-2xl animate-in slide-in-from-bottom-2 mb-2 w-56">
                                            {showBrushSettings && (
                                                <div className="space-y-4">
                                                    <div>
                                                        <label className="text-[9px] font-bold text-gray-500 uppercase flex justify-between">Size: <span className="text-white">{brushSize}px</span></label>
                                                        <input type="range" min="1" max="50" value={brushSize} onChange={(e) => setBrushSize(parseInt(e.target.value))} className="w-full h-1 bg-white/10 rounded-lg appearance-none cursor-pointer accent-violet-500 mt-2" />
                                                    </div>
                                                    <div>
                                                        <label className="text-[9px] font-bold text-gray-500 uppercase flex justify-between">Opacity: <span className="text-white">{Math.round(brushOpacity * 100)}%</span></label>
                                                        <input type="range" min="0.1" max="1" step="0.1" value={brushOpacity} onChange={(e) => setBrushOpacity(parseFloat(e.target.value))} className="w-full h-1 bg-white/10 rounded-lg appearance-none cursor-pointer accent-violet-500 mt-2" />
                                                    </div>
                                                </div>
                                            )}
                                            {showStampPicker && (
                                                <div>
                                                    <label className="text-[9px] font-bold text-gray-500 uppercase mb-3 block">Pick a Stamp</label>
                                                    <div className="grid grid-cols-5 gap-2">
                                                        {STAMPS.map(s => (
                                                            <button 
                                                                key={s} 
                                                                onClick={() => { setSelectedStamp(s); setDrawMode('stamp'); setShowStampPicker(false); }}
                                                                className={`w-8 h-8 rounded-lg flex items-center justify-center text-lg transition-all ${selectedStamp === s && drawMode === 'stamp' ? 'bg-violet-600 scale-110' : 'bg-white/5 hover:bg-white/10'}`}
                                                            >
                                                                {s}
                                                            </button>
                                                        ))}
                                    ...
                                    {/* Main Toolbar */}
                                    <div className="bg-[#1A1A23]/95 backdrop-blur-xl border border-white/10 rounded-[2rem] p-2 flex items-center gap-2 shadow-2xl shadow-black/40">
                                        <div className="flex items-center gap-1 border-r border-white/10 pr-2">
                                            <button onClick={() => { setDrawMode('pen'); setIsEraser(false); setShowStampPicker(false); }} className={`p-2 rounded-2xl transition-all ${drawMode === 'pen' && !isEraser ? 'bg-violet-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}><PenTool size={20}/></button>
                                            <button onClick={() => { setIsEraser(true); setShowBrushSettings(false); setShowStampPicker(false); }} className={`p-2 rounded-2xl transition-all ${isEraser ? 'bg-white text-black shadow-lg' : 'text-gray-400 hover:text-white'}`}><Eraser size={20}/></button>
                                            <button onClick={() => { setDrawMode('stamp'); setShowStampPicker(!showStampPicker); setShowBrushSettings(false); setIsEraser(false); }} className={`p-2 rounded-2xl transition-all ${drawMode === 'stamp' ? 'bg-yellow-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}><StickyNote size={20}/></button>
                                            <button onClick={() => { setShowBrushSettings(!showBrushSettings); setShowStampPicker(false); setIsEraser(false); }} className={`p-2 rounded-2xl transition-all ${showBrushSettings ? 'bg-fuchsia-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}><SlidersHorizontal size={20}/></button>
                                        </div>

                                        <div className="flex items-center gap-1 border-r border-white/10 pr-2">
                                            <button onClick={() => { setDrawMode('rect'); setIsEraser(false); setShowStampPicker(false); }} className={`p-2 rounded-2xl transition-all ${drawMode === 'rect' ? 'bg-violet-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}><Square size={20}/></button>
                                            <button onClick={() => { setDrawMode('circle'); setIsEraser(false); setShowStampPicker(false); }} className={`p-2 rounded-2xl transition-all ${drawMode === 'circle' ? 'bg-violet-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}><Circle size={20}/></button>
                                            <button onClick={() => { setDrawMode('line'); setIsEraser(false); setShowStampPicker(false); }} className={`p-2 rounded-2xl transition-all ${drawMode === 'line' ? 'bg-violet-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}><LineIcon size={20}/></button>
                                        </div>

                                        <div className="flex items-center gap-1 px-1">
                                            <button onClick={undoLastStroke} className="p-2 text-gray-400 hover:text-white transition-all active:scale-90" title="Undo"><Undo2 size={20} /></button>
                                            <div className="w-8 h-8 rounded-full border-2 border-white/20 relative overflow-hidden shadow-inner cursor-pointer group">
                                                <input type="color" value={brushColor} onChange={(e) => { setBrushColor(e.target.value); setIsEraser(false); }} className="absolute inset-0 w-[200%] h-[200%] -translate-x-1/4 -translate-y-1/4 cursor-pointer" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                 ...
                 <div className="flex-1 flex flex-col justify-center px-4 max-w-md mx-auto w-full">
                    <div className="flex justify-center mb-8 relative">
                        <div className="relative group cursor-pointer" onClick={(e) => handleSeatClick(999, e)}>
                            <div className="w-24 h-24 rounded-full border-4 border-yellow-500/30 bg-black/40 flex items-center justify-center relative overflow-hidden shadow-[0_0_30px_rgba(234,179,8,0.2)] transition-all active:scale-95">
                                {hostSeatOccupant ? (
                                    <>
                                    ...
                                    <Plus size={32} className="text-yellow-500/50" />
                                )}
                            </div>
                            ...
                            <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-yellow-500 text-black text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1 shadow-lg z-10 whitespace-nowrap"><Crown size={10} fill="black" /> {hostSeatOccupant ? hostSeatOccupant.displayName : 'Host Seat'}</div>
                        </div>
                    </div>
                 ...
                 <div className="h-48 px-4 pb-2 mt-4 mask-gradient-top overflow-y-auto no-scrollbar space-y-2">{messages.map((msg) => (<div key={msg.id} className={`text-xs animate-fade-in ${msg.type === 'system' ? 'text-yellow-400 font-medium text-center bg-yellow-400/5 py-1 rounded-lg' : msg.type === 'gift' ? 'text-center my-2' : 'text-white'}`}>{msg.type === 'system' ? (msg.text) : msg.type === 'gift' ? (<div className="inline-block bg-gradient-to-r from-violet-600/80 to-fuchsia-600/80 px-3 py-1.5 rounded-full border border-white/20 backdrop-blur-sm shadow-lg"><span className="font-bold text-white">{msg.senderName}</span> <span className="text-white/90">{msg.text}</span></div>) : (<div className="flex items-start gap-2 bg-black/40 p-2 rounded-xl w-fit max-w-[85%] backdrop-blur-sm border border-white/5"><img src={msg.senderPhoto || `https://ui-avatars.com/api/?name=${msg.senderName}`} className="w-5 h-5 rounded-full object-cover" /><div><span className="font-bold text-gray-400 mr-1.5 block leading-tight mb-0.5">{msg.senderName}</span><span className="text-gray-100 leading-snug">{msg.text}</span></div></div>)}</div>))}<div ref={chatEndRef} /></div>
            </div>
            ...
            <div className="px-3 pb-4 pt-2 relative z-30 flex items-center gap-2"><button onClick={() => setShowGiftModal(true)} className="p-2.5 bg-black/40 backdrop-blur-md border border-white/10 text-pink-400 rounded-full active:scale-95 flex-shrink-0 shadow-lg"><Gift size={20} /></button>{roomData?.musicState?.isEnabled && (<button onClick={() => setShowMusicModal(true)} className={`p-2.5 rounded-full flex-shrink-0 transition-all backdrop-blur-md shadow-lg ${roomData.musicState.isPlaying ? 'bg-violet-500 shadow-[0_0_15px_rgba(139,92,246,0.5)] animate-pulse-glow text-white' : 'bg-black/40 border border-white/10 text-white'}`}><Music2 size={20} /></button>)}{roomData?.drawingState?.isActive && !isArtist && (<button onClick={() => setIsBoardHidden(!isBoardHidden)} className={`p-2.5 rounded-full flex-shrink-0 backdrop-blur-md shadow-lg ${isBoardHidden ? 'bg-black/40 border border-white/10 text-gray-400' : 'bg-cyan-500 text-white shadow-[0_0_15px_rgba(6,182,212,0.5)]'}`}><PenTool size={20} /></button>)}<div className="flex-1 relative shadow-lg flex items-center bg-black/40 backdrop-blur-md border border-white/10 rounded-full pl-2 pr-2 py-1.5"><button onClick={() => setShowStickerPicker(!showStickerPicker)} className="p-1.5 text-yellow-400 hover:text-yellow-300 rounded-full transition-colors active:scale-95"><Smile size={20} /></button><form onSubmit={handleSendMessage} className="flex-1 flex items-center"><input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Say something..." className="w-full bg-transparent border-none outline-none pl-2 pr-2 py-1 text-xs text-white placeholder-gray-400" /><button type="submit" disabled={!newMessage.trim()} className="p-1.5 bg-violet-600 rounded-full text-white disabled:opacity-50 flex-shrink-0 ml-1"><Send size={12} fill="currentColor" /></button></form></div>{isOnSeat && (<button onClick={toggleMute} disabled={myPart?.isHostMuted} className={`p-2.5 rounded-full transition-all active:scale-95 border flex-shrink-0 shadow-lg backdrop-blur-md ${isMuted ? 'bg-black/40 text-white border-white/10' : 'bg-white text-black border-white shadow-[0_0_10px_rgba(255,255,255,0.4)]'}`}>{myPart?.isHostMuted ? <Lock size={20} className="text-red-500" /> : isMuted ? <MicOff size={20} /> : <Mic size={20} />}</button>)}</div>
            {showShareModal && (<div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in" onClick={() => setShowShareModal(false)}><div className="bg-[#18181B] w-full max-w-sm rounded-[2rem] border border-white/10 p-6 shadow-2xl" onClick={e => e.stopPropagation()}><div className="w-12 h-1.5 bg-white/10 rounded-full mx-auto mb-6"></div><h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><Share2 size={20} className="text-violet-400"/> Share Room</h3><div className="space-y-3 max-h-[50vh] overflow-y-auto pr-2">{recentChats.length === 0 ? (<p className="text-center text-gray-500 text-sm py-4">No recent chats.</p>) : (recentChats.map(chat => { const otherUser = chat.participantDetails.find(p => p.uid !== currentUser.uid); return (<button key={chat.id} onClick={() => inviteUserToRoom(chat.id)} className="w-full flex items-center justify-between p-3 bg-white/5 rounded-xl hover:bg-white/10 transition-colors"><div className="flex items-center gap-3"><img src={otherUser?.photoURL || ''} className="w-10 h-10 rounded-full bg-gray-800 object-cover" /><div className="text-left"><p className="font-bold text-sm text-white">{otherUser?.displayName}</p><p className="text-[10px] text-gray-400">Send Invite</p></div></div><div className="p-2 bg-violet-500/20 text-violet-400 rounded-lg"><Send size={16}/></div></button>) }))}</div></div></div>)}
            {showSettingsModal && (
                <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in" onClick={() => setShowSettingsModal(false)}>
                    <div className="bg-[#18181B] w-full max-w-sm rounded-[2rem] border border-white/10 p-6 shadow-2xl flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>
                        <div className="flex justify-between items-center mb-6"><h3 className="text-lg font-bold text-white flex items-center gap-2"><Settings size={20} className="text-gray-400"/> Room Settings</h3><button onClick={() => setShowSettingsModal(false)}><XIcon size={20} className="text-gray-500"/></button></div>
                        <div className="space-y-6 overflow-y-auto pr-2 no-scrollbar">
                            <div><label className="text-[10px] uppercase font-bold text-gray-500 tracking-wider">Room Name</label><input type="text" value={settingsName} onChange={(e) => setSettingsName(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 mt-1 text-sm text-white outline-none focus:border-violet-500" /></div>
                            <div><label className="text-[10px] uppercase font-bold text-gray-500 tracking-wider">Password (Optional)</label><input type="text" value={settingsPassword} onChange={(e) => setSettingsPassword(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 mt-1 text-sm text-white outline-none focus:border-violet-500" placeholder="4-digit code" /></div>
                            <div>
                                <label className="text-[10px] uppercase font-bold text-gray-500 tracking-wider mb-2 block">Choose Background</label>
                                <div className="grid grid-cols-2 gap-3">
                                    {availableBackgrounds.map(bg => (
                                        <button key={bg.id} onClick={() => setSettingsBg(bg.url)} className={`relative rounded-xl overflow-hidden border-2 aspect-video transition-all ${settingsBg === bg.url ? 'border-violet-500 scale-95 ring-2 ring-violet-500/30' : 'border-transparent hover:border-white/20'}`}>
                                            <img src={bg.url} className="w-full h-full object-cover" />
                                            {settingsBg === bg.url && <div className="absolute inset-0 bg-violet-600/20 flex items-center justify-center"><CheckCircle2 size={24} className="text-white drop-shadow-lg" /></div>}
                                        </button>
                                    ))}
                                    {availableBackgrounds.length === 0 && <p className="text-[10px] text-gray-500 col-span-2">No admin backgrounds available.</p>}
                                </div>
                            </div>
                        </div>
                        <button onClick={saveRoomSettings} className="w-full bg-violet-600 hover:bg-violet-500 text-white font-bold py-4 rounded-xl mt-6 shadow-lg shadow-violet-500/20">Save Changes</button>
                    </div>
                </div>
            )}
            ...
            {showRoomMenu && (<div className="absolute inset-0 z-40 bg-black/60 backdrop-blur-sm flex justify-end" onClick={() => setShowRoomMenu(false)}><div className="w-64 h-full bg-[#18181B] border-l border-white/10 p-6 animate-fade-in shadow-2xl" onClick={e => e.stopPropagation()}><h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-4">Room Menu</h3><div className="space-y-2">{(isHost || isAdmin) && (<MenuButton icon={musicEnabled ? <VolumeX size={20} /> : <Volume2 size={20} />} label={musicEnabled ? "Disable Music" : "Enable Music"} onClick={toggleMusicVisibility} />)}{(isHost || isAdmin) && (<MenuButton icon={<PenTool size={20} />} label="Drawing Board" onClick={() => { setShowRoomMenu(false); setShowDrawUserSelect(true); }} />)}<MenuButton icon={<Share2 size={20} />} label="Share Room" onClick={handleShareClick} /><MenuButton icon={<Users size={20} />} label="Viewer List" onClick={() => { setShowViewerList(true); setShowRoomMenu(false); }} /><MenuButton icon={isSpeakerOn ? <Volume2 size={20} /> : <VolumeX size={20} />} label={isSpeakerOn ? "Speaker On" : "Speaker Off"} onClick={toggleSpeaker} />{(isHost || isAdmin) && (<MenuButton icon={<Lock size={20} />} label="Room Settings" onClick={() => { setShowSettingsModal(true); setShowRoomMenu(false); }} />)}</div></div></div>)}
            ...
            {showDrawUserSelect && (<div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in" onClick={() => setShowDrawUserSelect(false)}><div className="bg-[#18181B] w-full max-w-sm rounded-[2rem] border border-white/10 p-6 shadow-2xl" onClick={e => e.stopPropagation()}><h3 className="text-lg font-bold text-white mb-4">Select Artist</h3><div className="space-y-2 max-h-[50vh] overflow-y-auto">{participants.map(p => (<button key={p.uid} onClick={() => selectDrawingUser(p)} className="w-full flex items-center gap-3 p-3 bg-white/5 rounded-xl hover:bg-white/10"><img src={p.photoURL || ''} className="w-10 h-10 rounded-full bg-gray-800 object-cover" /><span className="font-bold text-sm text-white">{p.displayName}</span></button>))}</div></div></div>)}
            {isDrawingPendingMe && (<div className="absolute top-20 left-4 right-4 z-50 bg-violet-600 text-white p-4 rounded-xl shadow-2xl flex items-center justify-between animate-bounce"><div><p className="font-bold text-sm">Drawing Invite</p><p className="text-xs text-white/80">Admin invited you to draw!</p></div><div className="flex gap-2"><button onClick={acceptDrawingInvite} className="px-3 py-1.5 bg-white text-violet-600 rounded-lg text-xs font-bold">Accept</button><button onClick={declineDrawingInvite} className="px-3 py-1.5 bg-black/20 text-white rounded-lg text-xs font-bold">Decline</button></div></div>)}
