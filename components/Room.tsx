...
const UserProfileModal: React.FC<{ 
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
            <div className="bg-[#1A1A21] w-full max-w-sm rounded-[2rem] p-6 border border-white/10 shadow-2xl animate-fade-in relative max-h-[80dvh] overflow-y-auto native-scroll" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex flex-col items-center">
                    ...
                    <h2 className="text-xl font-bold text-white flex items-center gap-2 text-center">{profile.displayName}
            ...
            <div className="relative w-14 h-14 sm:w-16 sm:h-16">
                ...
                {seat.occupant && seat.occupant.reaction && seat.occupant.reaction.expiresAt > Date.now() && (
                    <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
                        <img src={seat.occupant.reaction.url} className="w-20 h-20 drop-shadow-[0_8px_16px_rgba(0,0,0,0.8)] filter brightness-110 object-contain" />
                    </div>
                )}
            </div>
            <span className="text-[9px] text-gray-200 font-bold drop-shadow-md truncate w-14 text-center">{seat.occupant ? seat.occupant.displayName : `${seat.index + 1}`}</span>
        </div>
    );
    
    return (
        <div className={`h-[100dvh] w-full bg-[#050505] flex flex-col relative transition-all duration-300 overflow-hidden ${isMinimized ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
            ...
            <div className="flex items-center justify-between px-4 pb-2 pt-[calc(env(safe-area-inset-top)+0.5rem)] relative z-20 flex-shrink-0">
            ...
            {/* Main Content Area - Flexible to accommodate small screens */}
            <div className="relative flex-1 overflow-hidden z-10 flex flex-col min-h-0">
                        ...
                        <div className="relative group cursor-pointer w-20 h-20 sm:w-24 sm:h-24" onClick={(e) => handleSeatClick(999, e)}>
                            ...
                            {hostSeatOccupant && hostSeatOccupant.reaction && hostSeatOccupant.reaction.expiresAt > Date.now() && (
                                <div className="absolute inset-0 z-50 flex items-center justify-center pointer-events-none">
                                    <img src={hostSeatOccupant.reaction.url} className="w-24 h-24 sm:w-32 sm:h-32 drop-shadow-[0_10px_20px_rgba(0,0,0,0.8)] filter brightness-110 object-contain" />
                                </div>
                            )}
            ...
            <div className="px-3 pb-4 pt-2 pb-safe relative z-30 flex items-center gap-2 flex-shrink-0 bg-transparent">
