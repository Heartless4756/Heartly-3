            ...
            <div className="bg-[#1A1A21] w-full max-w-sm rounded-[2rem] p-6 border border-white/10 shadow-2xl animate-fade-in relative" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex flex-col items-center">
                    <div className="relative mb-4 w-24 h-24">
                        ...
                        {profile.frameUrl && <img src={profile.frameUrl} className="absolute inset-0 w-full h-full scale-[1.35] object-contain pointer-events-none" />}
          ...
          const unsubRoom = onSnapshot(roomRef, async (snapshot) => {
            if (snapshot.exists()) {
              ...
              if (!myPart) {
                 ...
                 const participant: Participant = { uid: currentUser.uid, displayName: currentUser.displayName || 'Guest', photoURL: currentUser.photoURL, frameUrl: currentUser.frameUrl, isMuted: true, isHostMuted: false, seatIndex: isCreator ? 999 : -1, joinedAt: Date.now(), lastSeen: Date.now() };
        ...
        <div key={seat.index} className="flex flex-col items-center gap-1 cursor-pointer relative" onClick={(e) => handleSeatClick(seat.index, e)}>
            <div className="relative w-14 h-14">
                ...
                {seat.occupant && seat.occupant.frameUrl && (
                    <img src={seat.occupant.frameUrl} className="absolute inset-0 w-full h-full scale-[1.35] object-contain pointer-events-none z-10" />
                )}
                    ...
                    <div className="flex justify-center mb-8 relative">
                        <div className="relative group cursor-pointer w-20 h-20" onClick={(e) => handleSeatClick(999, e)}>
                            ...
                            {hostSeatOccupant && hostSeatOccupant.frameUrl && (
                                <img src={hostSeatOccupant.frameUrl} className="absolute inset-0 w-full h-full scale-[1.35] object-contain pointer-events-none z-10" />
                            )}
