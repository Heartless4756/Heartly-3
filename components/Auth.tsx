...
export const Auth: React.FC = () => {
  ...
  const handleAuthSuccess = async (user: User, isGuest: boolean = false) => {
      try {
          ...
          if (!userDoc.exists()) {
              ...
              // For guests, generate a name like "Guest A1B2"
              ...
              await setDoc(userDocRef, {
                  ...
                  photoURL: user.photoURL, // Guests will have null, App handles UI avatar
                  uniqueId: uniqueId,
                  createdAt: Date.now(),
                  walletBalance: 100, // Starter Coins
                  followers: [],
                  following: [],
                  bio: isGuest ? 'Just visiting.' : 'Hey there! I am using Heartly.',
                  isAnonymous: isGuest
              });
          }
      } catch (e: any) {
        ...
        <div className="glass-card rounded-[2.5rem] p-8 shadow-2xl relative animate-fade-in bg-[#121216]/50 border border-white/5 backdrop-blur-xl">
          ...
          <div className="space-y-3">
              ...
              <button 
                ...
                disabled={loading}
                className="w-full bg-transparent border-2 border-white/10 hover:bg-white/5 text-gray-300 font-bold py-4 rounded-2xl transition-all active:scale-95 flex items-center justify-center gap-3"
              >
                  {loading ? (
                    <div className="flex items-center gap-2">
                        <Loader2 className="animate-spin" size={20} />
