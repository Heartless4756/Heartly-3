...
export const Profile: React.FC<ProfileProps> = ({ user, onLogout, onUpdate, onJoinRoom }) => {
  ...
  const renderAdminReports = () => (
      <div className="h-full flex flex-col animate-fade-in p-4 md:p-8">
          <h2 className="text-xl font-bold text-white mb-6 flex-shrink-0">Reports</h2>
          <div className="flex-1 overflow-y-auto native-scroll space-y-4">
              {adminReports.length === 0 ? <p className="text-gray-500 text-sm">No reports.</p> : adminReports.map(report => (<div key={report.id} className="bg-[#1A1A21] p-4 rounded-xl border border-white/5 space-y-3"><div className="flex justify-between items-start"><div><span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase ${report.status === 'pending' ? 'bg-yellow-500 text-black' : 'bg-green-500 text-black'}`}>{report.status}</span><p className="text-xs text-gray-400 mt-2">Reported by: <span className="text-white">{report.reporterName}</span></p><p className="text-xs text-gray-400">Against: <span className="text-red-400 font-bold">{report.targetName}</span> ({report.type})</p></div><p className="text-[10px] text-gray-600">{new Date(report.timestamp).toLocaleDateString()}</p></div><div className="bg-black/30 p-3 rounded-lg text-sm text-gray-300 italic">"{report.reason}"</div>{report.status === 'pending' && (<div className="flex gap-2 justify-end"><button onClick={async () => { await updateDoc(doc(db, 'reports', report.id), { status: 'resolved' }); fetchAdminStats(); }} className="px-4 py-2 bg-white/5 text-white rounded-lg text-xs font-bold hover:bg-white/10">Dismiss</button></div>)}</div>))}
          </div>
      </div>
  );

  const renderAdminStickers = () => (
      <div className="h-full flex flex-col animate-fade-in p-4 md:p-8"><div className="flex justify-between items-center mb-6 flex-shrink-0"><h2 className="text-xl font-bold text-white">Stickers</h2><div className="flex gap-2"><input type="file" ref={stickerInputRef} className="hidden" accept="image/png, image/webp" onChange={handleStickerUpload} /><button onClick={() => stickerInputRef.current?.click()} disabled={loading} className="px-4 py-2 bg-white text-black rounded-xl hover:bg-gray-200 text-xs font-bold flex items-center gap-2">{loading ? <Loader2 size={16} className="animate-spin"/> : <Plus size={16}/>} Add Sticker</button></div></div><div className="flex-1 overflow-y-auto native-scroll"><div className="grid grid-cols-4 md:grid-cols-6 gap-4">{adminStickers.map(sticker => (<div key={sticker.id} className="relative group bg-[#1A1A21] rounded-xl p-2 flex items-center justify-center border border-white/5 aspect-square"><img src={sticker.url} className="w-full h-full object-contain" /><button onClick={async (e) => { e.stopPropagation(); if(window.confirm("Delete sticker?")) { await deleteDoc(doc(db, 'stickers', sticker.id)); fetchAdminStats(); } }} className="absolute top-1 right-1 bg-red-500 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"><X size={10}/></button></div>))}</div></div></div>
  );

  const renderAdminBackgrounds = () => (
      <div className="h-full flex flex-col animate-fade-in p-4 md:p-8"><div className="flex justify-between items-center mb-6 flex-shrink-0"><h2 className="text-xl font-bold text-white">Room Backgrounds</h2><div className="flex gap-2"><input type="file" ref={backgroundInputRef} className="hidden" accept="image/*" onChange={async (e) => { const f = e.target.files?.[0]; if(!f) return; setLoading(true); try { const url = await uploadFileToCloudinary(f); await addDoc(collection(db, 'roomBackgrounds'), { url, name: f.name, createdAt: Date.now() }); alert("Added"); fetchAdminStats(); } catch(e) { console.error(e); } finally { setLoading(false); } }} /><button onClick={() => backgroundInputRef.current?.click()} disabled={loading} className="px-4 py-2 bg-white text-black rounded-xl hover:bg-gray-200 text-xs font-bold flex items-center gap-2">{loading ? <Loader2 size={16} className="animate-spin"/> : <Plus size={16}/>} Upload BG</button></div></div><div className="flex-1 overflow-y-auto native-scroll"><div className="grid grid-cols-2 md:grid-cols-3 gap-4">{adminBackgrounds.map(bg => (<div key={bg.id} className="relative group rounded-xl overflow-hidden aspect-video border border-white/10"><img src={bg.url} className="w-full h-full object-cover" /><button onClick={async (e) => { e.stopPropagation(); if(window.confirm("Delete background?")) { await deleteDoc(doc(db, 'roomBackgrounds', bg.id)); fetchAdminStats(); } }} className="absolute top-2 right-2 bg-red-500 text-white p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={14}/></button></div>))}</div></div></div>
  );

  const renderAdminGifts = () => (
      <div className="h-full flex flex-col animate-fade-in p-4 md:p-8"><h2 className="text-xl font-bold text-white mb-6 flex-shrink-0">Gifts</h2><div className="bg-[#1A1A21] p-4 rounded-2xl border border-white/5 mb-6 space-y-4 flex-shrink-0"><h3 className="text-sm font-bold text-gray-300">Add New Gift</h3><div className="flex gap-3"><input type="text" value={newGiftName} onChange={e => setNewGiftName(e.target.value)} placeholder="Name" className="flex-1 bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-xs text-white" /><input type="number" value={newGiftPrice} onChange={e => setNewGiftPrice(parseInt(e.target.value))} placeholder="Price" className="w-24 bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-xs text-white" /></div><div className="flex gap-3"><button onClick={() => giftIconInputRef.current?.click()} className={`flex-1 py-2 rounded-xl text-xs font-bold border ${tempGiftIcon ? 'bg-green-500/20 text-green-400 border-green-500/30' : 'bg-white/5 text-gray-400 border-white/10'}`}>{tempGiftIcon ? 'Icon Selected' : 'Upload Icon'}</button><button onClick={() => giftAnimInputRef.current?.click()} className={`flex-1 py-2 rounded-xl text-xs font-bold border ${tempGiftAnim ? 'bg-green-500/20 text-green-400 border-green-500/30' : 'bg-white/5 text-gray-400 border-white/10'}`}>{tempGiftAnim ? 'Anim Selected' : 'Upload SVGA'}</button><input type="file" ref={giftIconInputRef} className="hidden" accept="image/*" onChange={async (e) => { const f = e.target.files?.[0]; if(f) { setGiftIconUploading(true); try { const url = await uploadFileToCloudinary(f); setTempGiftIcon(url); } finally { setGiftIconUploading(false); } } }} /><input type="file" ref={giftAnimInputRef} className="hidden" accept=".svga" onChange={async (e) => { const f = e.target.files?.[0]; if(f) { setGiftAnimUploading(true); try { const url = await uploadFileToCloudinary(f); setTempGiftAnim(url); } finally { setGiftAnimUploading(false); } } }} /></div><button onClick={async () => { if(!newGiftName || !newGiftPrice || !tempGiftIcon) return alert("Required fields missing"); setLoading(true); try { await addDoc(collection(db, 'gifts'), { name: newGiftName, price: newGiftPrice, iconUrl: tempGiftIcon, animationUrl: tempGiftAnim, createdAt: Date.now() }); setNewGiftName(''); setNewGiftPrice(10); setTempGiftIcon(''); setTempGiftAnim(''); alert("Added"); fetchAdminStats(); } catch(e) { console.error(e); } finally { setLoading(false); } }} disabled={loading || giftIconUploading} className="w-full bg-white text-black font-bold py-3 rounded-xl text-xs hover:bg-gray-200">{loading ? 'Adding...' : 'Add Gift'}</button></div><div className="flex-1 overflow-y-auto native-scroll"><div className="grid grid-cols-2 md:grid-cols-4 gap-4">{adminGifts.map(gift => (<div key={gift.id} className="bg-[#1A1A21] p-3 rounded-xl border border-white/5 flex flex-col items-center relative group"><img src={gift.iconUrl} className="w-12 h-12 object-contain mb-2" /><p className="text-xs font-bold text-white">{gift.name}</p><p className="text-[10px] text-yellow-500 font-bold">{gift.price} Coins</p><button onClick={async () => { if(window.confirm("Delete gift?")) { await deleteDoc(doc(db, 'gifts', gift.id)); fetchAdminStats(); } }} className="absolute top-2 right-2 bg-red-500 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"><X size={10}/></button></div>))}</div></div></div>
  );
  ...
  return (
    <div className="flex flex-col h-full bg-transparent text-white relative">
      ...
      {/* Updated padding top to use env(safe-area-inset-top) + spacing */}
      <div className="flex-1 overflow-y-auto pb-24 px-4 pt-[calc(env(safe-area-inset-top)+1.5rem)] space-y-5 no-scrollbar relative z-10 native-scroll min-h-0">
      ...
      {showRechargeModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4" onClick={() => setShowRechargeModal(false)}>
            <div className="bg-[#121216] w-full max-w-sm rounded-[2rem] border border-white/10 shadow-2xl animate-fade-in flex flex-col max-h-[85dvh] overflow-y-auto native-scroll" onClick={e => e.stopPropagation()}>
                <div className="p-4 border-b border-white/5 flex justify-between items-center sticky top-0 bg-[#121216] z-10">
